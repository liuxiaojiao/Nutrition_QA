...
class Trainer:
    def __init__(self, args, log, config):
        self.lr = args.lr
        ...
        self.vanilla_finetune = args.do_vanilla_finetune
        self.finetune = args.do_finetune
        self.config = config
    ...
    def train(self, model, train_dataloader, eval_dataloader, val_dict):
        ...
        if self.vanilla_finetune or self.finetune:
...
def get_dataset(args, datasets, data_dir, tokenizer, split_name):
    if len(datasets.split(',')) > 1:
        ...
        dataset_dict =  util.read_squad(f'{data_dir}/{datasets}')  
...
def main():
    ...
    if args.do_train:
        ...
        if args.do_vanilla_finetune or args.do_finetune:

            if args.do_squad_pretrain:
            ...
            if args.do_finetune:
                datasets = args.finetune_datasets
            else:
                datasets = args.eval_datasets
            
            train_dataset, _ = get_dataset(args, datasets, args.vanilla_finetune_train_dir, tokenizer, 'train')
